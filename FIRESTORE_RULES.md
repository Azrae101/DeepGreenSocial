...
**Solution:** Update your Firestore rules to allow unauthenticated users to read all posts (since there's no approval system implemented).
...
service cloud.firestore {
  match /databases/{database}/documents {
    // Allow anyone (authenticated or not) to read all posts
    match /posts/{postId} {
      allow read: if true;
...
- ✅ **Anyone** (authenticated or not) can **read all posts**
- ✅ Only **logged-in users** can **create new posts**
- ✅ Only **admins and post authors** can **update/delete posts**
...
2. You should see all posts
3. Login to create new posts and interact with likes/comments
